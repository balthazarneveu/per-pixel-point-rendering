...
# from interactive_pipe.data_objects.image
...
def main(out_root=OUT_DIR, name=STAIRCASE):
    ...
    wc_points, points_colors, wc_normals = pick_point_cloud_from_triangles(
        wc_triangles, colors, wc_normals, num_samples=400)
    for idx, yaw_angle in enumerate(range(-30, 30, 5)):
        ...
        Image(splatted_image).save(out_dir/f"{idx:04}.png")
