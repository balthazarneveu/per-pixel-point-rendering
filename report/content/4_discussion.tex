...
\subsubsection*{View dependent materials}.
We could provide extra inputs to the neural network to model view dependent materials. By providing the angle between the point normal and the view direction (these 2 vectors are computed during the normal culling test). These 2 angles could be concatenated to the projected pseudo colors vectors (and we'd probalby need to map these to sine positional embeddings as proposed in NERF  \cite{mildenhall2020nerf}). 
\textit{I eventually found out about NPBG++ \cite{rakhimov2022npbg} where the authors propose to fit the coefficients of spherical harmonics function .}